...
defmodule Ex04 do
  ...
  def min(list) do
    [ first | _ ] = list
    reduce(list, first, &(if &1 < &2, do: &1, else: &2))
  end
  ...
  def even_odd_func(x, {stateLeft, stateRight}) do
    cond do
      Integer.is_even x -> {[x | stateLeft], stateRight}
      Integer.is_odd  x -> {stateLeft, [x | stateRight]}
    end
  end

  def even_odd(x) do
     {l, r} = reduce(x, {[],[]}, &even_odd_func/2)
     {reverse(l), reverse(r)}
  end
